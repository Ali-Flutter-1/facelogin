...
/// Handles BIP39 recovery phrase generation, hashing, and PBKDF2 key derivation
class RecoveryKeyService {
  ...
  static const List<String> _bip39WordList = [
    ...
    'rip', 'ripe', 'rise', 'risk', 'ritual', 'rival', 'river', 'road',
    ...
    'shovel', 'show', 'shrimp', 'shrug', 'shuffle', 'shy', 'sibling', 'sick',
  ...
  /// Returns space-separated phrase
  static String generateRecoveryPhrase() {
    final random = Random.secure();
    final words = <String>[];
    
    for (int i = 0; i < 12; i++) {
      final index = random.nextInt(_bip39WordList.length);
  ...
  /// Hash recovery phrase using SHA-256
  /// Returns base64-encoded hash
  static String hashRecoveryPhrase(String recoveryPhrase) {
    final bytes = utf8.encode(recoveryPhrase);
    final hash = sha256.convert(bytes);
    return base64Encode(hash.bytes);
  }

  /// Derive AES-256 key from recovery phrase using PBKDF2
  /// Uses SHA-256 as PRF, 100,000 iterations, 32-byte key
  /// Uses fixed salt for consistency (same salt every time)
  static Uint8List deriveAESKeyFromPhrase(String recoveryPhrase) {
    // Use PBKDF2 with SHA-256
    // Fixed salt: same salt used every time for consistency
    // This ensures backend and frontend generate the same recovery key
    final saltBytes = utf8.encode('recovery_salt_fixed_2024');
    final saltHash = sha256.convert(saltBytes);
    final salt = Uint8List.fromList(saltHash.bytes.sublist(0, 16)); // 16-byte salt
    ...
    final phraseBytes = utf8.encode(recoveryPhrase);
    final key = pbkdf2.process(phraseBytes);
    
    return Uint8List.fromList(key);
  }

  /// Encrypt data with recovery key using AES-GCM
  /// Returns IV (12 bytes) + ciphertext
  static Uint8List encryptWithRecoveryKey(Uint8List data, String recoveryPhrase) {
    final key = deriveAESKeyFromPhrase(recoveryPhrase);
  ...
  /// Decrypt data with recovery key using AES-GCM
  /// Expects IV (12 bytes) + ciphertext
  static Uint8List decryptWithRecoveryKey(Uint8List encryptedData, String recoveryPhrase) {
  ...
  /// Validate recovery phrase format (12 words, all in BIP39 list)
  static bool isValidRecoveryPhrase(String phrase) {
    ...
    for (final word in words) {
      if (!_bip39WordList.contains(word.toLowerCase())) {
        return false;
      }
    }
